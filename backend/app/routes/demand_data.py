...
def get_demand_data():
    ...
    try:
        ...
        if start_date:
            query = query.filter(DemandData.timestamp >= datetime.fromisoformat(start_date))
        if end_date:
            query = query.filter(DemandData.timestamp <= datetime.fromisoformat(end_date))
        if weather:
        ...
        if isinstance(data, list):
            ...
            for item in data:
                ...
                if 'station_id' in item:
                    ...
                    if not station:
                        return jsonify({'error': f"站点ID {item['station_id']} 不存在"}), 400
...
def get_demand_statistics():
    ...
    try:
        ...
        avg_demand = db.session.query(db.func.avg(DemandData.demand)).filter(*query.statement.where_clause.clauses).scalar() or 0
        max_demand = db.session.query(db.func.max(DemandData.demand)).filter(*query.statement.where_clause.clauses).scalar() or 0
        min_demand = db.session.query(db.func.min(DemandData.demand)).filter(*query.statement.where_clause.clauses).scalar() or 0

        # 按站点类型统计
        station_type_stats = db.session.query(
            DemandData.station_type,
            db.func.count(DemandData.id).label('count'),
            db.func.avg(DemandData.demand).label('avg_demand')
        ).filter(*query.statement.where_clause.clauses).group_by(DemandData.station_type).all()
        ...
        weather_stats = db.session.query(
            ...
            db.func.count(DemandData.id).label('count'),
            db.func.avg(DemandData.demand).label('avg_demand')
        ).filter(*query.statement.where_clause.clauses).group_by(DemandData.weather).all()
